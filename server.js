...
app.get('/patient/:cpf/latest_metrics', (req, res) => {
    ...
    // Query to get all the health metrics for the specific patient, ordered by time
    const sql = 'SELECT epoch, ind_card, ind_pulm FROM health_data WHERE cpf = ? ORDER BY epoch';
    ...
    db.all(sql, [cpf], (err, rows) => {
        ...
        // Format the results
        const { ind_pulm } = rows.find(({ ind_pulm }) => ind_pulm);
        const { ind_card } = rows.find(({ ind_card }) => ind_card);
...
// Endpoint 7: Get the latest patients with pagination and optional name search
app.get('/patients/search', (req, res) => {
    ...
    // Query to get the total number of patients with optional name filtering
    const countSql = 'SELECT COUNT(*) as total FROM patients WHERE nome LIKE ?';

    // Query to get the patients data with pagination and optional name filtering
    const dataSql = 'SELECT * FROM patients WHERE nome LIKE ? ORDER BY cpf DESC LIMIT ? OFFSET ?';

    // First, get the total number of patients
    db.get(countSql, [`%${name}%`], (err, countResult) => {
        if (err) {
            return res.status(500).json({ error: err.message });
        }

        const totalPatients = countResult.total;
        const totalPages = Math.ceil(totalPatients / limit);

        // Next, get the patients data with pagination
        db.all(dataSql, [`%${name}%`, limit, offset], (err, rows) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
            return res.json({
